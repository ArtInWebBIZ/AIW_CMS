...
class Func
{
    ...
    public function checkAccess(): bool
    {
        if ($this->checkAccess === null) {
            ...
            if (
                ...
                (
                    (int) $this->getReview()['status'] === ForAll::compIncFile('Review', 'status')['REVIEW_PUBLISHED'] ||
                    (
    ...
    private function editAccess()
    {
        if (
            (
                (int) $this->getReview()['status'] === ForAll::compIncFile('Review', 'status')['REVIEW_NOT_PUBLISHED'] &&
                (int) $this->getReview()['author_id'] === Auth::getUserId()
            ) ||
            GroupAccess::check([5])
