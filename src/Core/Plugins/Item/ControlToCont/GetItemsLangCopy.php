...
class GetItemsLangCopy
{
    ...
    public static function getI(): GetItemsLangCopy
    {
    ...
    private function getItems(): array
    {
        ...
        if (Func::getI()->itemCount() > 0) {
            ...
            foreach (Func::getI()->itemParams()['fields_in_body'] as $key => $value) {
                if (in_array($value, Fields::getAllItemFields()['default'])) {
                    $fields['item'][] = $value;
                } elseif (in_array($value, Fields::getAllItemFields()['filters'])) {
                    $fields['filters'][] = $value;
                } elseif (in_array($value, Fields::getAllItemFields()['fieldset'])) {
            ...
            if (isset($fields['filters'])) {
                $filtersTable = 'item_' . str_replace("-", "_", Func::getI()->itemParams()['controller_url']);
                ...
                foreach ($item as $itemKey => $itemValue) {
                    foreach ($fieldset as $fieldsetName => $fieldsetValue) {
                        foreach ($fieldsetValue as $key => $value) {
                            ...
                            if (
                                (int) $fieldsetValue[$key]['item_id'] === (int) $item[$itemKey]['id']
                            ) {
    ...
    public function getItemsLang(): array
    {
        if (Func::getI()->itemCount() > 0) {
            ...
            if (Func::getI()->itemParams()['lang_fields'] != []) {
                $variable = Func::getI()->itemParams()['lang_fields'];
