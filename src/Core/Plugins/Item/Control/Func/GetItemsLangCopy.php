    ...
    private function getItems(): array
    {
        if ($this->getItems === null) {
            ...
            if (Func::getI()->itemCount() > 0) {
                ...
                $allItemsId = Func::getI()->getAllItemsId();
                $in      = ParamsToSql::getInSql($allItemsId);
                $inIn    = $in['in'];
                $inArray = $in['array'];
                ...
                foreach ($item as $key => $value) {
                    $id = $item[$key]['item_id'];
                    $itemOk[$id] = $item[$key];
                    unset($item[$key], $itemOk[$id]['item_id']);
                }
                $item = $itemOk;
                unset($key, $value, $itemOk);
                ...
                if (isset($fields['fieldset'])) {
                    ...
                    foreach ($item as $itemId => $itemValue) {
                        $item[$itemId]['id'] = $itemId;
                        foreach ($fieldset as $name => $array) {
                            foreach ($array as $key => $value) {
                                if ((int) $value['item_id'] === $itemId) {
                                    $item[$itemId][$name][] = $value[$name];
                                    unset($array[$key]);
                                }
                            }
                        }
                    }
                    unset($itemId, $itemValue, $fieldset, $name, $array, $key, $value);
                }
